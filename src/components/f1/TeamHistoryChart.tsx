...
interface Props {
  history: TeamSeasonStat[];
  teamColor: string;
}

export function TeamHistoryChart({ history, teamColor }: Props) {
  ...
  const maxPts = Math.max(...sorted.map((s) => s.points), 1);
  ...
  return (
    <div className="bg-[#141420] border border-[#2D2D3A] rounded-xl p-5">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
        <defs>
          <linearGradient id={`team-grad-${teamColor.replace("#", "")}`} x1="0" y1="0" x2="0" y2="1">
        ...
        {/* Area fill */}
        <polygon points={areaPoints} fill={`url(#team-grad-${teamColor.replace("#", "")})`} />
        ...
        {sorted.map((s) => {
          ...
          return (
            <g key={s.season}>
              ...
              {isChampion && <circle cx={x} cy={y} r={7} fill={teamColor} opacity={0.2} />}
      ...
      <p className="text-[10px] text-[#475569] mt-2">Jolpica(Ergast) 데이터 기준 · 점 위 숫자 = 해당 시즌 우승 수</p>
    </div>
  );
}
